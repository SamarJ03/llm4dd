...
class Log:
    ...
    def setLog(
        ...
        verbose: str = os.getenv("VERBOSE", "INFO"),
        debug: Optional[bool] = None,
        trackMem: Optional[bool] = None,
    ) -> Any:
        try:
            debug = (
                debug
            ...
            trackMem = (
                trackMem
                if trackMem is not None
                else os.getenv("TRACK_MEM", "false").lower() == "true"
            )
            ...
            for k, v in self._config.get("app", {}).get("paths", {}).items():
                if isinstance(v, str) and v.startswith("&"):
                    ...
                    if anchor in value:
                        ...
                        if anchor_value:
                            value = value.replace(f"&{anchor}", anchor_value)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="l4d", description="Command line interface for LLM4DD"
    )
    ...
    if args.api_keys == "load":
        ...
        if keys:
            ...
            for key in sec.list():
                logger.debug(f"keys found: {sec.list()}")
        else:
        ...
        while True:
            ...
            if provider == "q":
                ...
                print(
                    f"Invalid provider. Please enter API keys from the supported providers: {accepted}"
                )
            ...
            keys_set += 1
        if keys_set == 0:
            raise Exception(f"No keys set..")
        else:
            logger.info(f"Succesfully set LLM API keys..")

    if str(args.source_path).lower() != "local":
        if not Path(args.source_path).exists():
            raise FileExistsError(f"file not found:{args.source_path}")
