...
class DataPrep:
    ...
    def ConfigureRawData(self, sourcePath: str = None) -> pl.DataFrame:
        ...
        if source_path is None:
            raise ValueError(
                f'"SOURCE_PATH" not included in arguments or found in .env'
            )
        if not os.path.exists(source_path):
            raise FileNotFoundError(
                f"source_path is not a valid file path: {source_path}"
            )

        _, ext = os.path.splitext(source_path)
        ext = ext.lower()
        if ext == ".csv":
            df = pl.read_csv(source_path, has_header=True)
        elif ext in [".xlsx", ".xls"]:
            df = pl.read_excel(source_path, has_header=True)
        elif ext == ".json":
            ...
            raise NotImplementedError(
                f"Source file path is of an unsupported type: [{ext}]"
            )
        ...
        if isinstance(df, pl.DataFrame):
            try:
                ...
                opt = pl.DataFrame(
                    df.select([
                        optCol
                        for optCol in ["pathway", "target", "info"]
                        if optCol in df.columns
                    ])
                )
                rawData = req if not opt else pl.concat([req, opt], how="horizontal")
                rawData.write_csv(
                    os.path.join(self.data_path, "raw_lib.csv"), include_header=True
                )
                set_key(
                    os.getenv("ENV_PATH"),
                    "RAWDATA_PATH",
                    os.path.join(self.data_path, "raw_lib.csv"),
                )
                return True
            except Exception as e:
                raise Exception(f"Error in ConfigureRawData(): {e}")
    ...
    def getRawData(self) -> pl.DataFrame:
        ...
        if not os.path.exists(path) or path is None:
            path = os.path.join(self.data_path, "raw_lib.csv")
        if not os.path.exists(path) or path is None:
    ...
    def standardize(self, rawData: pl.DataFrame = None):
        ...
        def addFeatures(self, smi: str, nameKey: str) -> dict:
            ...
            try:
                ...
                log.debug(
                    f"!-{nameKey}: feature extraction: could not configure Chem.Mol"
                )
            if not mol:
                log.debug(
                    f"!-{nameKey}: feature extraction: could not configure Chem.Mol"
            ...
            try:
                ...
                log.debug(f"!-{nameKey}: feature extraction [rdkit]: {e}")
            try:
                maccsSer = pl.Series(
                ...
                log.debug(f"!-{nameKey}: feature extraction [maccs]: {e}")
            try:
                ecfp4Ser = pl.Series(
                    values=list(
                ...
                log.debug(f"!-{nameKey}: feature extraction [ecfp4]: {e}")
        ...
        def normalize(self, nameKey, smi) -> tuple[bool, str]:
            ...
            try:
                ...
                return False, str(e)
            try:
                cleanedMol = rdMolStandardize.Cleanup(mol)
            except Exception as e:
                log.debug(
                    f"!-{nameKey}: failed normalization (H+ removal, metal atom disconnection, mol normalization+reionization)"
                )
                return False, str(e)
            try:
                parentMol = rdMolStandardize.FragmentParent(cleanedMol)
            except Exception as e:
                log.debug(
                    f"!-{nameKey}: failed normalization (retrieving parent fragment)"
                )
                return False, str(e)
            try:
                unchargedMol = self.uncharger.uncharge(parentMol)
            except Exception as e:
                log.debug(f"!-{nameKey}: failed normalization (mol neutralization)")
                return False, str(e)
            try:
                tautMol = self.te.Canonicalize(unchargedMol)
            except Exception as e:
                log.debug(
                    f"!-{nameKey}: failed normalization (tautomer canonicalization)"
                )
                return False, str(e)
            try:
                newsmi = Chem.MolToSmiles(tautMol)
            except Exception as e:
        ...
        log = self.logger
        parsed = {
            "nan": {"compound_name": [], "SMILES": []},
            "non-standardized": {},
            "duplicate": {},
            "invalid_score": {},
        }
        if rawData is not None:
            df = pl.DataFrame(rawData)
        else:
            df = pl.read_csv(os.path.join(self.data_path, "raw_lib.csv"))
        if not {"compound_name", "SMILES", "score"}.issubset(df.columns):
            raise ValueError("Missing required columns")
        else:
            df = df.cast({"compound_name": str, "SMILES": str, "score": float})
        ...
        for row in df.iter_rows():
            ...
            if name is None or name.lower() in ["", "nan", "none", "na"]:
                name = "(default)"
            if smi is None or smi.lower() in [
                "",
                "nan",
                "none",
                "na",
            ]:  # remove nan SMILES value
                parsed["nan"]["SMILES"].append(f"[{index}]_{name}")
                log.debug(f"!-[{index}]_{name}: NaN removal (nan smiles)\n")
                continue
            if pScore is None or str(pScore).lower() in [
                "",
                "nan",
                "none",
                "na",
            ]:  # remove nan score value
            ...
            if not dupRow.is_empty or dupRow is not None:
                oldName = str(dupRow["compound_name"].item())
                oldSmi = str(dupRow["SMILES"].item())
                oldScore = float(dupRow["score"].item())
                if pScore < oldScore:  # drop previous iteration
                    lib = lib.remove(
                        compound_name=oldName, SMILES=oldSmi, score=oldScore
                    )
            ...
            rdkit.append(
                pl.concat([row, pl.Series(features["rdkit"])], how="horizontal")
            )
            maccs.append(
                pl.concat([row, pl.Series(features["maccs"])], how="horizontal")
            )
            ecfp4.append(
                pl.concat([row, pl.Series(features["ecfp4"])], how="horizontal")
            )

        log.info(f"completed data standardization and SMILES normalization.")
        lib.write_csv(os.path.join(self.data_path, "lib.csv"))
        with open(
            os.path.join(self.base_path, "logs", "filtered_compounds.json"), "w"
        ) as f:
    ...
    def curateFeatures(
        ...
        df: pl.DataFrame,
        type: str,
        vt: float = float(os.getenv("VARIANCE_THRESHOLD", "0.1")),
    ):
        log = self.logger
        if type not in ["rdkit", "maccs", "ecfp4"]:
            log.error("dataset type not supported: {type}")
        colToDrop = [
            "compound_name",
            "SMILES",
            "score",
        ...
        infoDF = pl.DataFrame(
            df.select([col for col in colToDrop if col in df.columns])
        )
        out = pl.DataFrame(
            df.drop([col for col in colToDrop if col in df.columns])
        ).cast(int if type != "rdkit" else float)
        outnp = out.to_numpy()
        ...
        currPath = os.path.join(self.scafPath, type)
        ...
        def split(
            ...
            name: str,
            path,
            rs: int = int(os.getenv("RANDOM_STATE", "15")),
        ):
            drop_cols = ["compound_name", "score", "pathway", "target", "info"]
            train, temp = train_test_split(
                df, test_size=0.2, random_state=rs, stratify=df["label"], shuffle=True
            )
            ...
            pl.DataFrame(train).drop(
                columns=[col for col in drop_cols if col in train.columns]
            ).to_csv(f"{path}/{name}_train.csv", index=False)
            pl.DataFrame(test).drop(
                columns=[col for col in drop_cols if col in test.columns]
            ).to_csv(f"{path}/{name}_test.csv", index=False)
            pl.DataFrame(valid).drop(
                columns=[col for col in drop_cols if col in valid.columns]
            ).to_csv(f"{path}/{name}_valid.csv", index=False)

        def splitSubcats(name, path, sc: dict = None):
            for key, val in sc.items():
                if str(key) == "all":
                    split(df=val, name=name, path=path)
                else:
        ...
        if type == "rdkit":
            ...
            mtCols = [
                ...
                for col in fgcCols
                if str(col).lower()
                in ["balabanj", "bertzct", "hallkieralpha", "ipc", "avgipc"]
            ]
            fbCols = [
                col
                for col in mtCols
                if any(str(col).startswith(prefix) for prefix in ["Fp", "BCUT2D"])
            ]
            saCols = [
                col
                for col in fbCols
                if any(x in str(col).lower() for x in ["peoe", "smr", "slogp"])
                or str(col).lower() == "labuteasa"
            ]
            sdcCols = [
                col
                for col in saCols
                if (
                    str(col).lower().startswith("n")
                    and str(col).lower()
                    not in ["numvalenceelectrons", "numradicalelectrons"]
                )
                or str(col).lower() in ["fractioncsp3", "ringcount"]
            ]
            pcCol = [
                col
                for col in descs.to_list()
                if col not in estateCols + fgcCols + mtCols + fbCols + saCols + sdcCols
            ]
            subcategories = {
                "all": pl.DataFrame(meta),
                "EState": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(estateCols)], how="horizontal"
                ),
                "functionalGroupCounts": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(fgcCols)], how="horizontal"
                ),
                "molecularTopology": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(mtCols)], how="horizontal"
                ),
                "fingerprintBased": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(fbCols)], how="horizontal"
                ),
                "surfaceArea": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(saCols)], how="horizontal"
                ),
                "structural": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(sdcCols)], how="horizontal"
                ),
                "physiochemical": pl.concat(
                    [infoDF, pl.DataFrame(out).filter(pcCol)], how="horizontal"
                ),
            }
            splitSubcats(name=type, path=currPath, sc=subcategories, rs=self.rs)
        else:
            split(df=meta, name=type, path=currPath, rs=self.rs)
